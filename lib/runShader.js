export function runShader(
	...
	while (true) {
		...
		for (let i = start, n = 0; i < end; i += 4, n++) {
			const x = (n % tileSizeX) + tileX * tileSizeX;
			const y = Math.trunc(n / tileSizeX) + tileY * tileSizeY;

			const clipSpaceX = Math.fround((x + 0.5) / (uniformsbuffer[1] / 2) - 1);
			const clipSpaceY = Math.fround(1 - (y + 0.5) / (uniformsbuffer[2] / 2));

			shader(color, clipSpaceX, clipSpaceY, uniformsbuffer);
